				...
				if ( $result ) {
					...
					for ($i=0 ; $i<$count_init ; $i++) {
						if ($current_occurrence[$i]["id"] == "selectPostWithID".$post->ID) {
							...
							// Si on a pas trouvÈ l'occurrence dans $tl["occ"], cela veut dire qu'il a ÈtÈ supprimÈ, on le supprime aussi de $current_occurrence
							} else {
	...
	*/
	...
	function _modify_content_callback($matches) {
		...
		global $post ; 
  		...
  		// $matches[1] represente la premi√®re parenth√®se capturante
